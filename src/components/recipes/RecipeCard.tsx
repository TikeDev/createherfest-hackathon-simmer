    ...
    <li>
      ...
      >
        <div className="flex items-start justify-between gap-2">
          <h2 className="font-headline text-forest text-sm leading-snug">{recipe.title}</h2>
        ...
        <div className="mt-1 flex flex-wrap gap-3 text-xs text-forest/50">
          ...
          {recipe.metadata.totalTimeMinutes && <span>{recipe.metadata.totalTimeMinutes} min</span>}
          {recipe.metadata.servings && <span>Serves {recipe.metadata.servings}</span>}
          <span>{recipe.ingredients.length} ingredients</span>
          <span>{recipe.steps.length} steps</span>
        </div>
        {reason && <p className="mt-2 text-xs text-sage font-medium">{reason}</p>}
      </Link>
    </li>
  );
}
