    ...
    steps: [
      {
        id: "s1",
        index: 1,
        text: "Cook the vegetables until soft.",
        isCritical: false,
        annotations: [],
      },
      {
        id: "s2",
        index: 2,
        text: "Use an immersion blender to blend until smooth.",
        isCritical: false,
        annotations: [],
      },
    ],
    ...
    steps: [
      {
        id: "s3",
        index: 1,
        text: "Combine all ingredients in a pot and simmer for 30 minutes.",
        isCritical: false,
        annotations: [],
      },
    ],
...
describe("time preference soft scoring", () => {
  const quickRecipe = makeRecipe({
    id: "quick",
    title: "Quick Eggs",
    cognitiveScore: 1,
    totalTimeMinutes: 15,
  });
  const slowRecipe = makeRecipe({
    id: "slow",
    title: "Slow Braise",
    cognitiveScore: 1,
    ...
    steps: [
      {
        id: "s1",
        index: 1,
        text: "Add everything to the slow cooker and cook on low for 6 hours.",
        isCritical: false,
        annotations: [],
      },
    ],
    ...
    steps: [
      {
        id: "s2",
        index: 1,
        text: "Heat a pan and fry the tofu until golden.",
        isCritical: false,
        annotations: [],
      },
    ],
...
describe("match reasons", () => {
  it("always includes an energy level reason", () => {
    ...
    const hasEnergyReason = results[0].matchReasons.some(
      (r) => r.includes("Low-complexity") || r.includes("Medium") || r.includes("More involved")
    );
    expect(hasEnergyReason).toBe(true);
  });

  it("includes total time when available", () => {
    const timedRecipe = makeRecipe({
      id: "t",
      title: "Timed Recipe",
      cognitiveScore: 1,
      totalTimeMinutes: 40,
      ...
      steps: [
        {
          id: "s1",
          index: 1,
          text: "Blend the soup until smooth.",
          isCritical: false,
          annotations: [],
        },
      ],
...
describe("edge cases", () => {
  ...
  it("handles a very long query without error", () => {
    ...
    expect(() =>
      suggestRecipes(longQuery, 3, [easyRecipe, mediumRecipe, hardRecipe])
    ).not.toThrow();
  });
