...
export function computeCognitiveScore(recipe: RecipeJSON): 1 | 2 | 3 {
  ...
  const activeTimingCount = steps.filter(
    (s) =>
  ...
  const normalized = {
    ...
    criticalSteps: normalize(
      criticalCount,
      THRESHOLDS.criticalSteps.low,
      THRESHOLDS.criticalSteps.high
    ),
    techniqueAnnotations: normalize(
      techniqueCount,
      THRESHOLDS.techniqueAnnotations.low,
      THRESHOLDS.techniqueAnnotations.high
    ),
    activeTimingSteps: normalize(
      activeTimingCount,
      THRESHOLDS.activeTimingSteps.low,
      THRESHOLDS.activeTimingSteps.high
    ),
    ingredientCount: normalize(
      ingredientCount,
      THRESHOLDS.ingredientCount.low,
      THRESHOLDS.ingredientCount.high
    ),
  };
  ...
  // Map to 1–3 output
  if (rawScore <= 0.4) return 1;
  if (rawScore <= 0.7) return 2;
  return 3;
}
...
export function computeCognitiveScoreDebug(recipe: RecipeJSON): {
  ...
  const score: 1 | 2 | 3 = raw <= 0.4 ? 1 : raw <= 0.7 ? 2 : 3;
