...
function buildUserMessage(
  ...
  const recipeList = recipes
    .map(r => {
      ...
      const topIngredients = r.ingredients.slice(0, 6).map(i => i.name).join(', ')
...
export async function runSuggestionAgent(
  ...
  return (parsed.suggestions ?? [])
    .filter(s => typeof s.recipeId === 'string' && validIds.has(s.recipeId) && typeof s.reason === 'string' && s.reason.length > 0)
    .slice(0, 3)
    .map(s => ({
